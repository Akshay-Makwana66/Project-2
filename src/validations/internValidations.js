...
const internValidations = async function (req, res,next) {
  try { 
    ...
    if (!college.mobile)return res.status(400).send({ status: false, msg: "Please Enter Mobile Number" });
     if (typeof college.mobile !== "number")return res.status(400).send({ status: false, msg: " Please enter only number of 10 digits & don't write number in string" });
    ...
    if (!validInternCollegeName.test(internCollegeName.collegeName))return res.status(400).send({ status: false, msg: "The internCollegeName may contain only small letters & not space between words or letter & numbers as well"});
